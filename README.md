...
**Why there is no file upload, and why that is a decision rather than a gap.**
Every number this tool reports its own accuracy with — precision, recall, the
false-match rate, the split between confirmed and flagged — is computed *against
published ground truth*. Ground truth exists because the batch is generated by a
model that knows what it planted. Arbitrary uploaded data has no ground truth, so
against it none of those metrics can be computed at all: the tool could still
print findings, but it could no longer tell you which of them are verified and
which are merely flagged. That distinction is the whole claim. An upload button
would trade the one property that makes this credible for a demo affordance, so
